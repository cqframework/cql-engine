...
//        }
        ...
        ArrayList<AliasedQuerySource> sources = (ArrayList<AliasedQuerySource>) this.getSource();
        ArrayList<Object> sourceObjects = new ArrayList<>();
        boolean sourceIsList = false;
        for (AliasedQuerySource source : sources) {
            Object element = source.getExpression().evaluate(context);
            if (element instanceof Iterable) {
                ...
                for (Object e : (Iterable)element) {
                    sourceObjects.add(e);
                    try {
                        context.push(new Variable().withName(source.getAlias()).withValue(e));
