  ...
  public Object evaluate(Context context) {
    ...
    if (left != null && right != null) {
      ...
      if (leftStart instanceof Integer) {
        return ((Integer)rightStart == (Integer)leftEnd + 1);
      }

      else if (leftStart instanceof BigDecimal) {
        return (((BigDecimal)rightStart).compareTo(((BigDecimal)leftEnd).add(new BigDecimal(1))) == 0);
      }

      else if (leftStart instanceof Quantity) {
        return ((((Quantity)rightStart).getValue()).compareTo((((Quantity)leftEnd).getValue()).add(new BigDecimal(1))) == 0);
      }
